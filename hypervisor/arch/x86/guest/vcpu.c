...
static struct acrn_vcpu_regs rm_init_regs = {
	.gdt = {
		.limit = 0xFFFFU,
		.base = 0UL,
	},
...
{
	set_vcpu_regs(vcpu, &rm_init_regs);
}
...
/* NOTE: set_bsp_rm_entry & set_bsp_protect_regs is only temporary function
 * called by UOS. Once we make UOS to use hypercall to set BSP entry,
 ...
 */
void set_bsp_rm_entry(struct vcpu *vcpu)
{
	struct ext_context *ectx;
...
static struct acrn_vcpu_regs protect_init_regs = {
...
void set_bsp_protect_regs(struct vcpu *vcpu)
{
	/* if vcpu is in protect mode, we need to set registers
	 * according to protect_init_regs first.
	 */
	uint32_t limit;
	protect_init_regs.gdt.base = create_guest_init_gdt(vcpu->vm,
			&limit);
	protect_init_regs.gdt.limit = (uint16_t) (limit & 0xFFFFU);
	set_vcpu_regs(vcpu, &protect_init_regs);
